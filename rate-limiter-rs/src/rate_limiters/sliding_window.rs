...
impl RateLimiter for SlidingWindowRateLimiter {
    fn check_request(
        ...
        let (request_count, oldest_request_in_window_epoch_time): (u64, Vec<String>) =
        ...
        println!("request_count={} most_recent={:?}", request_count, oldest_request_in_window_epoch_time);

        let oldest_request_in_window_epoch_time: u64 = oldest_request_in_window_epoch_time[0]
            .parse()
            .map_err(|_e| RateLimiterError::ComputeError)?;
        ...
        let response = if request_count <= self.window_size {
            ...
            //let retry_in = current_ts_epoch_time - most_recent_entry_epoch_time;
            let time_passed_from_first_req = Duration::from_nanos(current_ts_epoch_time - oldest_request_in_window_epoch_time);
